...
public class PortGenerator {
     ...
     * @return
     */
    public static synchronized Integer getRedisPort(final String ip,String userName,String passWard) {
        ...
        try {
            ...
            maxPortStr = getMaxPortStr(ip, sshPort,userName,passWard);
...
//            while (SSHUtil.isPortUsed(ip, availablePort)) {
            //modify by jiangcy for different passwd
//            logger.warn("is port used:",SSHUtil.isPortUsed(ip,availablePort,userName,passWard));
            while (SSHUtil.isPortUsed(ip,availablePort,userName,passWard)) {
                availablePort++;
            }
        } catch (SSHException e) {
    ...
    @Deprecated
    public static String getMaxPortStrOld(String ip, int sshPort,String userName,String passWard) throws SSHException {
        String redisPidCmd = "ps -ef | grep redis | grep -v 'grep' |  awk -F '*:' '{print $2}' " +
                " | awk -F ' ' '{print $1}' | sort -r | head -1";

        return SSHUtil.execute(ip, sshPort, userName, passWard, redisPidCmd);
    }
     ...
     * @throws SSHException
     */
     public static String getMaxPortStr(String ip, int sshPort,String userName,String passWard) throws SSHException {
        String redisPidCmd = "ps -ef | grep redis | grep -v 'grep'";

        String redisProcessStr = SSHUtil.execute(ip, sshPort, userName, passWard, redisPidCmd);
//        String redisProcessStr = SSHUtil.execute(ip, sshPort, ConstUtils.USERNAME, ConstUtils.PASSWORD, redisPidCmd);


//        logger.warn("redisProcessStr : {}",redisProcessStr);
